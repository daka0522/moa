  ...
  methods: {
    listPublished() {
      ...
      indexRef.get().then((querySnapshot) => {
        querySnapshot.forEach((doc: Record<string, unknown | any>) => {
          let cont = {
            id: doc.id,
            data: doc.data(),
            show: false,
...
$test-first: hsla(270, 100%, 30%, 0.5);
$test-second: hsla(90, 100%, 30%, 0.5);
...
section {
  ...
  h1 {
    ...
    background: linear-gradient(
      23.5deg,
      rgba($test-first, 1) 0%,
      rgba($test-second, 1) 100%
    );
  }
}
article {
  padding: 0.62vh 0.62vw;
  ...
  .fullcontent {
    background-color: rgba(navajowhite, 0.25);
    border-radius: 0.5em;
    padding: 1rem 0;
    margin: 1rem 0;
...
#home-logo {
  margin-top: 1.6em;
  border-radius: 0.5em;
  box-shadow: 0 0 0.5em 0.1em rgba(128, 128, 128, 0.55);
}
</style>
